...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    assertions = [{
      ...
      message =
        "services.pods.auth requires services.pods.reverse-proxy to be enabled (for the reverse_proxy network)";
    }];
    ...
    home-manager.users.poddy = { pkgs, config, ... }: {
      ...
      in {
        volumes.authelia = { volumeConfig = { }; };
        volumes.lldap = { volumeConfig = { }; };
        ...
        containers.authelia = lib.mkIf cfg.authelia.enable {
          ...
          containerConfig = {
            ...
            labels = {
              ...
              "traefik.http.services.authelia.loadbalancer.server.scheme" =
                "http";
              "traefik.http.services.authelia.loadbalancer.server.port" =
                "9091";
              "traefik.http.middlewares.authelia.forwardauth.address" =
                "http://host.docker.internal:9091/api/authz/forward-auth";
              "traefik.http.middlewares.authelia.forwardauth.trustforwardheader" =
                "true";
            ...
            environments = {
              ...
              AUTHELIA_NOTIFIER_SMTP_ADDRESS =
                "submission://smtp.mail.me.com:587";
        ...
        containers.lldap = lib.mkIf cfg.lldap.enable {
          ...
          containerConfig = {
            ...
            labels = {
              ...
              "traefik.tcp.routers.ldap.tls.domains[0].main" =
                "lldap1.nmsd.xyz";
    ...
    sops.templates."auth-secrets" = {
      content = ''
        AUTHELIA_NOTIFIER_SMTP_PASSWORD=${
          config.sops.placeholder."authelia/icloud_smtp_password"
        }
        AUTHELIA_IDENTITY_VALIDATION_RESET_PASSWORD_JWT_SECRET=${
          config.sops.placeholder."authelia/authelia_jwt_secret"
        }
        AUTHELIA_SESSION_SECRET=${
          config.sops.placeholder."authelia/authelia_session_secret"
        }
        ...
        AUTHELIA_AUTHENTICATION_BACKEND_LDAP_PASSWORD=${
          config.sops.placeholder."ldap/lldap_ldap_user_pass"
        }
