            ...
            containers.paperless-web = {
              ...
              containerConfig = {
                ...
                environments = {
                  ...
                  # The container starts as root (so init-tesseract-langs can apt
                  # install OCR packs), then drops to the paperless user. Map that
                  # user's gid to 0, which in this rootless pod maps back to host
                  # poddy (1001) — the owning group of the bind-mounted consume dir
                  # (/data/scans, mode 2770 rwxrws---) — so consumed files can be
                  # deleted after import. The image's init-modify-user runs
                  # `groupmod --non-unique --gid 0`, so reusing root's gid is fine.
                  USERMAP_UID = "1000";
                  USERMAP_GID = "0";
