...
in
{
  options.services.pods.auth.authelia.configFile = lib.mkOption {
    ...
    default = yaml.generate "configuration.yml" {
      theme = "auto";
      ...
      totp.issuer = "${authCfg.authelia.subdomain}.${domain}";
      ...
      notifier = {
        ...
        smtp = {
          # url and username are secrets (delivered via env vars from sops)
      ...
      identity_providers.oidc = {
        ...
        # jwks is NOT in config - handled via env vars + sops file mounts:
        #   AUTHELIA_IDENTITY_PROVIDERS_OIDC_JWKS_0_KEY_ID
        #   AUTHELIA_IDENTITY_PROVIDERS_OIDC_JWKS_0_KEY_FILE
        #   AUTHELIA_IDENTITY_PROVIDERS_OIDC_JWKS_0_CERTIFICATE_CHAIN_FILE

        # client_secret is NOT in config - handled via env vars from sops:
